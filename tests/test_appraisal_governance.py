...
async def test_v1_snapshot_migrates_to_legacy_unreconciled(client, project):
    ...
    calc_version 2.13.0, and finance.requires_confirmation True in the stored
    (migrated) snapshot."""
    v1_snapshot = {
    ...
    assert body["calc_version"] == "2.17.0"
    ...
    # drive the schedule, exactly as they did before R12. R13 Task 18 extends
    ...
    # about the takeout or cost-to-complete path changes from R13. R13b Task
    ...
    # takeout or cost-to-complete path changes from R14. R15 Task 13 extends
    # it to v13 (spec Sec 23.10): a v1 document seeds due_diligence unknown
    # (there is nothing to migrate from). R15b Task 6 extends it to v14 (spec
    # Sec 24.8): a v1 document has no cost_plan.qs either, so it stays null
    # and nothing about the build cost changes from R15. R16 Task 4 extends
    # it to v15 (spec Sec 25.7): a v1 document's four scenario lever fields
    # are written at the same zero `ScenarioOverrides` already defaults them
    # to, so nothing about any figure changes from R15b.
    ...
    assert body["inputs_snapshot"]["unit_sales"] is None
    ...
    assert body["outputs"]["metrics"]["calc_version"] == "2.17.0"
...
# 18 (spec Sec 19.9) moved it to v10; R14 Task 14 (spec Sec 20.1) moved it to
# v11; R13b Task 15 (spec Sec 22.9) moves it to v12.
#
# Every unit test in this repo can pass with the server still normalising to
# v11 -- migrate_inputs_to_v12 is tested directly, the unit_sales block is
...
# rather than the proof that it could.
# ---------------------------------------------------------------------------
...
async def test_saved_appraisal_round_trips_as_v9(client, project):
    ...
    class docstring makes the same call for the same reason -- renaming every
    test that outlives the version in its name would obscure what each one
    pins.)
    ...
    assert resp.status_code == 201, resp.text
    created = resp.json()
    assert created["inputs_version"] == 15
    assert created["inputs_snapshot"]["inputs_version"] == 15

    fetched = (await client.get(f"/api/v1/appraisals/{project['id']}")).json()
    assert fetched["inputs_version"] == 15
    assert fetched["inputs_snapshot"]["inputs_version"] == 15
    ...
    # statement entered (spec Sec 20.1). Its unit_sales block is null too --
    # no per-unit ledger entered (spec Sec 22).
    ...
    assert fetched["inputs_snapshot"]["unit_sales"] is None
    # A v12 document is not a legacy migration. `is_v2_or_later` had to learn
    # is_v12 in R13b (ahead of this cutover, spec Sec 22.9): without it every
    # appraisal saved from the v12 calculator would come back stamped
...
async def test_resaving_the_v9_document_the_server_returned_is_not_legacy(client, project):
    ...
    back, post that. The second POST carries a v12 snapshot, which is the
    ...
    assert second.json()["inputs_version"] == 15
