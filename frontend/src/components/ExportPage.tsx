...
import { runAppraisal, migrateInputsToV9 } from '../lib/model';
...
export default function ExportPage({ projects, projectsLoading, backendOffline }: ExportPageProps) {
  ...
  const handleAppraisalPdf = useCallback(async () => {
    ...
    try {
      ...
      if (raw && typeof raw === 'object' && 'acquisition' in raw && 'unit_mix' in raw) {
        ...
        // §17.11) moved it to v8; R12 Task 18b (spec §18.7) moves it to v9, in
        // the SAME commit as the server -- each vN entry point throws on a
        // v(N+1) document (spec §3.5's guard against the v1-fallback
        // corruption path), so this must track the server boundary exactly or
        // every export throws.
        spider = computeSpider(migrateInputsToV9(normaliseUnitAreas(raw), selectedProject), eligibility);
      }
  ...
  const handleInvestmentMemo = useCallback(async () => {
    ...
    try {
      ...
      // moved this to v7; R11 Task 10 moved it to v8; R12 Task 18b moves it to
      // v9, matching the server boundary, which moves in the same commit. This
      // is also where a stored v8 explicit programme first reaches the memo's
      ...
      const run = runAppraisal(migrateInputsToV9(normaliseUnitAreas(raw), selectedProject));
