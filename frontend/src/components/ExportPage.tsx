...
import { runAppraisal, migrateInputsToV12 } from '../lib/model';
...
export default function ExportPage({ projects, projectsLoading, backendOffline }: ExportPageProps) {
  ...
  const handleAppraisalPdf = useCallback(async () => {
    ...
    try {
      ...
      if (raw && typeof raw === 'object' && 'acquisition' in raw && 'unit_mix' in raw) {
        ...
        // it to v11; R13b Task 15 (spec §22.9) moves it to v12, in the SAME
        // commit as the server -- each vN entry point throws on a v(N+1)
        // document (spec §3.5's guard against the v1-fallback corruption
        // path), so this must track the server boundary exactly or every
        // export throws.
        spider = computeSpider(migrateInputsToV12(normaliseUnitAreas(raw), selectedProject), eligibility);
      }
  ...
  const handleInvestmentMemo = useCallback(async () => {
    ...
    try {
      ...
      // Task 15 moves it to v12, matching the server boundary, which moves in
      // the same commit. R12 Task 18b was also where a stored v8 explicit
      ...
      const run = runAppraisal(migrateInputsToV12(normaliseUnitAreas(raw), selectedProject));
