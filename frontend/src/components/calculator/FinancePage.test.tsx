...
import type { CalculatorInputsV16, MonitoringInputs } from '../../lib/model';
...
describe('FinancePage — lender valuation entry card wiring', () => {
  it('shows the "no lender valuation recorded" empty state when the block is absent', () => {
    const inputs: CalculatorInputsV16 = { ...defaultCalculatorInputsV16(), lender_valuation: null };
    setup(inputs);
  ...
  it('renders the enforcement cost assumption field with its current value', () => {
    const inputs: CalculatorInputsV16 = {
      ...defaultCalculatorInputsV16(),
      finance: { ...defaultCalculatorInputsV16().finance, funding_source: 'development_finance', enforcement_cost_assumption_pence: 50_000 },
    };
    setup(inputs);
...
describe('FinancePage — MonitoringEditor mount wiring (R14 fix wave finding 3a)', () => {
  // term_months defaults to 12 (see defaultCalculatorInputsV16's finance block),
  ...
  function inputsWithBothIssues(): CalculatorInputsV16 {
    return {
      ...defaultCalculatorInputsV16(),
