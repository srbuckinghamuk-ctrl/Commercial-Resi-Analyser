...
describe('FinancePage — lender valuation entry card wiring', () => {
  it('shows the "no lender valuation recorded" empty state when the block is absent', () => {
    const inputs: CalculatorInputsV8 = { ...defaultCalculatorInputsV8(), lender_valuation: null };
    setup(inputs);
  ...
  it('renders the enforcement cost assumption field with its current value', () => {
    const inputs: CalculatorInputsV8 = {
      ...defaultCalculatorInputsV8(),
      finance: { ...defaultCalculatorInputsV8().finance, funding_source: 'development_finance', enforcement_cost_assumption_pence: 50_000 },
    };
    setup(inputs);
