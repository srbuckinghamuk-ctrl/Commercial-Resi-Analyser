...
function setup(inputs: CalculatorInputsV3, onChange = vi.fn()) {
...
describe('FinancePage — lender valuation entry card wiring', () => {
  it('shows the "no lender valuation recorded" empty state when the block is absent', () => {
    const inputs: CalculatorInputsV3 = { ...defaultCalculatorInputsV3(), lender_valuation: null };
    setup(inputs);
  ...
  it('surfaces lender_valuation validation errors from the live run on the entry card', () => {
    const inputs: CalculatorInputsV3 = {
  ...
  it('renders the enforcement cost assumption field with its current value', () => {
    const inputs: CalculatorInputsV3 = {
      ...defaultCalculatorInputsV3(),
      finance: { ...defaultCalculatorInputsV3().finance, funding_source: 'development_finance', enforcement_cost_assumption_pence: 50_000 },
    };
    setup(inputs);
