...
import type { CalculatorInputsV11, MonitoringInputs } from '../../lib/model';
...
describe('FinancePage — lender valuation entry card wiring', () => {
  it('shows the "no lender valuation recorded" empty state when the block is absent', () => {
    const inputs: CalculatorInputsV11 = { ...defaultCalculatorInputsV11(), lender_valuation: null };
    setup(inputs);
  ...
  it('surfaces lender_valuation validation errors from the live run on the entry card', () => {
    const inputs: CalculatorInputsV11 = {
  ...
  it('renders the enforcement cost assumption field with its current value', () => {
    const inputs: CalculatorInputsV11 = {
      ...defaultCalculatorInputsV11(),
      finance: { ...defaultCalculatorInputsV11().finance, funding_source: 'development_finance', enforcement_cost_assumption_pence: 50_000 },
    };
    setup(inputs);
...
describe('FinancePage — MonitoringEditor mount wiring (R14 fix wave finding 3a)', () => {
  // term_months defaults to 12 (see defaultCalculatorInputsV11's finance block),
  ...
  function inputsWithBothIssues(): CalculatorInputsV11 {
    return {
      ...defaultCalculatorInputsV11(),
