...
import type { CalculatorInputsV17, MonitoringInputs } from '../../lib/model';
...
describe('FinancePage — lender valuation entry card wiring', () => {
  it('shows the "no lender valuation recorded" empty state when the block is absent', () => {
    const inputs: CalculatorInputsV17 = { ...defaultCalculatorInputsV17(), lender_valuation: null };
    setup(inputs);
  ...
  it('renders the enforcement cost assumption field with its current value', () => {
    const inputs: CalculatorInputsV17 = {
      ...defaultCalculatorInputsV17(),
      finance: { ...defaultCalculatorInputsV17().finance, funding_source: 'development_finance', enforcement_cost_assumption_pence: 50_000 },
    };
    setup(inputs);
...
describe('FinancePage — MonitoringEditor mount wiring (R14 fix wave finding 3a)', () => {
  // term_months defaults to 12 (see defaultCalculatorInputsV17's finance block),
  ...
  function inputsWithBothIssues(): CalculatorInputsV17 {
    return {
      ...defaultCalculatorInputsV17(),
