...
import type {
  ...
  CalculatorInputsV12, CalculatorInputsV13, CalculatorInputsV14, CalculatorInputsV15, CalculatorInputsV16,
  CalculatorInputsV17, ProgrammeInputs, ProgrammeNetwork, Phase,
} from '../../lib/model';
 ...
 * wired it to v15; R16b Task 2 wired it to v16; R17 Task 3 (the entry-point
 * cutover) wires it to v17, so
 * T now resolves to CalculatorInputsV17 at the real call site
 * (ConversionCalculator.tsx) and the legacy `{ packages: {...} }` arm below
 * is reachable only from a caller still holding a v8 document -- the tests,
 * and any future one. `programme` is unchanged in shape from v9 through v17
 * (only `refinance`, `investment_case`, `monitoring`, `unit_sales`,
 * `due_diligence`, `cost_plan.qs.inflation`, the four stress-pack scenario
 * fields, at v16 the narrowed `conversion_costs` and, at v17, the nullable
 * `elemental_benchmark` block, `CostPackage.benchmark_origin` and the
 * due-diligence source records differ across those versions), so widening
 * the union costs this page nothing. Props stays
 * generic over every shape `programme` can legally live on, and `onChange`
 * stays typed to whichever one the caller has, so all ten compile against
 * the SAME component. Keeping the v8 arm is deliberate: it is what proves
 * the legacy discriminator still works, and it costs nothing at the v17 call
 * site.
 */
