 ...
 * to v12; R15 Task 13 (the entry-point cutover) wires it to v13, so T now
 * resolves to CalculatorInputsV13 at the real call site
 ...
 * and any future one. `programme` is unchanged in shape from v9 through v13
 * (only `refinance`, `investment_case`, `monitoring`, `unit_sales` and
 * `due_diligence` differ across those versions), so widening the union costs
 * this page nothing. Props stays generic over every shape `programme` can
 * legally live on, and `onChange` stays typed to whichever one the caller
 * has, so all six compile against the SAME component. Keeping the v8 arm is
 * deliberate: it is what proves the legacy discriminator still works, and it
 * costs nothing at the v13 call site.
 */
