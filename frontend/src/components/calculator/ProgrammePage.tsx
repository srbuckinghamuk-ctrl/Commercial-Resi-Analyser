 ...
 * wired it to v15; R16b Task 2 (the entry-point cutover) wires it to v16, so
 * T now resolves to CalculatorInputsV16 at the real call site
 ...
 * and any future one. `programme` is unchanged in shape from v9 through v16
 * (only `refinance`, `investment_case`, `monitoring`, `unit_sales`,
 * `due_diligence`, `cost_plan.qs.inflation`, the four stress-pack scenario
 * fields and, at v16, the narrowed `conversion_costs` differ across those
 * versions), so widening the union costs this page nothing. Props stays
 * generic over every shape `programme` can legally live on, and `onChange`
 * stays typed to whichever one the caller has, so all nine compile against
 * the SAME component. Keeping the v8 arm is deliberate: it is what proves
 * the legacy discriminator still works, and it costs nothing at the v16 call
 * site.
 */
