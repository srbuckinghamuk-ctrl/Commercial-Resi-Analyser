...
import type { AppraisalRun, CalculatorInputsV10, CostPackage, FeeLine } from '../../lib/model';
 ...
 */
function baseInputs(basis: 'manual' | 'bridge_derived'): CalculatorInputsV10 {
...
// checking.
function detailedInputs(): CalculatorInputsV10 {
  const base = defaultCalculatorInputsV10();
  return {
    ...base,
...
describe('ConversionCostsPage — reads cost figures from run.metrics.cost_plan, never recomputes them', () => {
  ...
  it('shows the package grid in detailed mode and hides it in headline mode', () => {
    ...
    cleanup();
    const headline: CalculatorInputsV10 = {
      ...inputs,
  ...
  it('offers to convert compliance allowances into a package when switching to detailed mode', () => {
    const base = defaultCalculatorInputsV10();
    const inputs: CalculatorInputsV10 = {
      ...base,
      conversion_costs: {
        ...base.conversion_costs,
  ...
  it('switches mode without prompting when there is no compliance to convert', () => {
    const inputs = defaultCalculatorInputsV10(); // compliance fields are 0 by default
    const run = runAppraisal(inputs);
    const onChange = vi.fn();
...
describe('ConversionCostsPage — the return trip cannot lose money (C2, fix round 1)', () => {
  ...
  it('allows switching back to headline once every package is zeroed', () => {
    const inputs = detailedInputs();
    const zeroed: CalculatorInputsV10 = {
      ...inputs,
      cost_plan: {
        ...inputs.cost_plan,
...
function feeTestInputs(feeLines: FeeLine[]): CalculatorInputsV10 {
  const base = defaultCalculatorInputsV10();
  return {
    ...base,
