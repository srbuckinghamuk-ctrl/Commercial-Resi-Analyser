...
import type { AppraisalRun, CalculatorInputsV11, CostPackage, FeeLine } from '../../lib/model';
 ...
 */
function baseInputs(basis: 'manual' | 'bridge_derived'): CalculatorInputsV11 {
...
// checking.
function detailedInputs(): CalculatorInputsV11 {
  const base = defaultCalculatorInputsV11();
  return {
    ...base,
...
describe('ConversionCostsPage — reads cost figures from run.metrics.cost_plan, never recomputes them', () => {
  ...
  it('shows the package grid in detailed mode and hides it in headline mode', () => {
    ...
    cleanup();
    const headline: CalculatorInputsV11 = {
      ...inputs,
  ...
  it('switches mode without prompting when there is no compliance to convert', () => {
    const inputs = defaultCalculatorInputsV11(); // compliance fields are 0 by default
    const run = runAppraisal(inputs);
    const onChange = vi.fn();
...
describe('ConversionCostsPage — the return trip cannot lose money (C2, fix round 1)', () => {
  ...
  it('allows switching back to headline once every package is zeroed', () => {
    const inputs = detailedInputs();
    const zeroed: CalculatorInputsV11 = {
      ...inputs,
      cost_plan: {
        ...inputs.cost_plan,
...
function feeTestInputs(feeLines: FeeLine[]): CalculatorInputsV11 {
  const base = defaultCalculatorInputsV11();
  return {
    ...base,
