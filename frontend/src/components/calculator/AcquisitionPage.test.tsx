...
// 18's V9->V10 one, and for R14 Task 14's V10->V11 one). AcquisitionPage is
// now typed on CalculatorInputsV11, so every fixture is migrated once, here,
...
// an already-v9 one, `migrateV9toV10` an already-v10 one and `migrateV10toV11`
// an already-v11 one, so the day memo-fixtures.ts is widened to return
// CalculatorInputsV11 -- making these calls redundant -- every wrapper below
// throws immediately and every test in this file fails at setup, not a silent
// double-migration or a quietly wrong document.
function welshInputs(): CalculatorInputsV11 {
  return migrateV10toV11(migrateV9toV10(migrateV8toV9(migrateV7toV8(migrateV6toV7(welshInputsV6())))));
}
function scottishInputs(): CalculatorInputsV11 {
  return migrateV10toV11(migrateV9toV10(migrateV8toV9(migrateV7toV8(migrateV6toV7(scottishInputsV6())))));
}
function unconfirmedJurisdictionInputs(): CalculatorInputsV11 {
  return migrateV10toV11(migrateV9toV10(migrateV8toV9(migrateV7toV8(migrateV6toV7(unconfirmedJurisdictionInputsV6())))));
}
...
function derivedUnconfirmed(base: CalculatorInputsV11): CalculatorInputsV11 {
  return {
    ...base,
    acquisition: {
      ...base.acquisition,
...
function setup(inputs: CalculatorInputsV11, onChange = vi.fn()) {
...
describe('AcquisitionPage — the jurisdiction control (R8 defect B)', () => {
  ...
  // defaultCalculatorInputsV11 deliberately records no jurisdiction of its own.
  it('says a defaulted jurisdiction was never recorded, and flags it unconfirmed', () => {
    const base = welshInputs();
    const migrated: CalculatorInputsV11 = {
      ...base,
      acquisition: {
        ...base.acquisition,
  ...
  it('does not claim a confirmed basis when the acquisition date is still outstanding', () => {
    const base = welshInputs();
    const noDate: CalculatorInputsV11 = {
...
describe('AcquisitionPage — the acquisition date', () => {
  ...
  it('surfaces the engine\'s own error for a date no band set covers', () => {
    const base = welshInputs();
    const tooEarly: CalculatorInputsV11 = {
...
describe('AcquisitionPage — the tax override', () => {
  function overridden(pence: number | null, reason: string): CalculatorInputsV11 {
    const base = welshInputs();
    return {
      ...base,
      acquisition: {
