...
// V11->V12 one, and for R15 Task 13's V12->V13 one). AcquisitionPage is now
// typed on CalculatorInputsV13, so every fixture is migrated once, here,
// rather than widening the shared fixture file for one caller.
//
...
// an already-v11 one, `migrateV11toV12` an already-v12 one, and
// `migrateV12toV13` an already-v13 one, so the day memo-fixtures.ts is
// widened to return CalculatorInputsV14 -- making these calls redundant --
...
function welshInputs(): CalculatorInputsV13 {
  return migrateV12toV13(migrateV11toV12(migrateV10toV11(migrateV9toV10(migrateV8toV9(migrateV7toV8(migrateV6toV7(welshInputsV6())))))));
}
function scottishInputs(): CalculatorInputsV13 {
  return migrateV12toV13(migrateV11toV12(migrateV10toV11(migrateV9toV10(migrateV8toV9(migrateV7toV8(migrateV6toV7(scottishInputsV6())))))));
}
function unconfirmedJurisdictionInputs(): CalculatorInputsV13 {
  return migrateV12toV13(migrateV11toV12(migrateV10toV11(migrateV9toV10(migrateV8toV9(migrateV7toV8(migrateV6toV7(unconfirmedJurisdictionInputsV6())))))));
}
...
function derivedUnconfirmed(base: CalculatorInputsV13): CalculatorInputsV13 {
  return {
    ...base,
    acquisition: {
      ...base.acquisition,
...
function setup(inputs: CalculatorInputsV13, onChange = vi.fn()) {
...
describe('AcquisitionPage — the jurisdiction control (R8 defect B)', () => {
  ...
  // defaultCalculatorInputsV13 deliberately records no jurisdiction of its own.
  it('says a defaulted jurisdiction was never recorded, and flags it unconfirmed', () => {
    const base = welshInputs();
    const migrated: CalculatorInputsV13 = {
      ...base,
      acquisition: {
        ...base.acquisition,
  ...
  it('does not claim a confirmed basis when the acquisition date is still outstanding', () => {
    const base = welshInputs();
    const noDate: CalculatorInputsV13 = {
...
describe('AcquisitionPage — the acquisition date', () => {
  ...
  it('surfaces the engine\'s own error for a date no band set covers', () => {
    const base = welshInputs();
    const tooEarly: CalculatorInputsV13 = {
...
describe('AcquisitionPage — the tax override', () => {
  function overridden(pence: number | null, reason: string): CalculatorInputsV13 {
    const base = welshInputs();
    return {
      ...base,
      acquisition: {
