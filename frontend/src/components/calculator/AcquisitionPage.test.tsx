...
// 18's V9->V10 one, for R14 Task 14's V10->V11 one, and for R13b Task 15's
// V11->V12 one). AcquisitionPage is now typed on CalculatorInputsV12, so
...
// an already-v11 one, and `migrateV11toV12` an already-v12 one, so the day
// memo-fixtures.ts is widened to return CalculatorInputsV12 -- making these
...
// wrong document.
function welshInputs(): CalculatorInputsV12 {
  return migrateV11toV12(migrateV10toV11(migrateV9toV10(migrateV8toV9(migrateV7toV8(migrateV6toV7(welshInputsV6()))))));
}
function scottishInputs(): CalculatorInputsV12 {
  return migrateV11toV12(migrateV10toV11(migrateV9toV10(migrateV8toV9(migrateV7toV8(migrateV6toV7(scottishInputsV6()))))));
}
function unconfirmedJurisdictionInputs(): CalculatorInputsV12 {
  return migrateV11toV12(migrateV10toV11(migrateV9toV10(migrateV8toV9(migrateV7toV8(migrateV6toV7(unconfirmedJurisdictionInputsV6()))))));
}
...
function derivedUnconfirmed(base: CalculatorInputsV12): CalculatorInputsV12 {
  return {
    ...base,
    acquisition: {
      ...base.acquisition,
...
describe('AcquisitionPage — the jurisdiction control (R8 defect B)', () => {
  ...
  // defaultCalculatorInputsV12 deliberately records no jurisdiction of its own.
  it('says a defaulted jurisdiction was never recorded, and flags it unconfirmed', () => {
    const base = welshInputs();
    const migrated: CalculatorInputsV12 = {
      ...base,
      acquisition: {
        ...base.acquisition,
  ...
  it('does not claim a confirmed basis when the acquisition date is still outstanding', () => {
    const base = welshInputs();
    const noDate: CalculatorInputsV12 = {
...
describe('AcquisitionPage — the acquisition date', () => {
  ...
  it('surfaces the engine\'s own error for a date no band set covers', () => {
    const base = welshInputs();
    const tooEarly: CalculatorInputsV12 = {
...
describe('AcquisitionPage — the tax override', () => {
  function overridden(pence: number | null, reason: string): CalculatorInputsV12 {
    const base = welshInputs();
    return {
      ...base,
      acquisition: {
