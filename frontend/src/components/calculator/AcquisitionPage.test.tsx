...
// V11->V12 one, for R15 Task 13's V12->V13 one, and for R15b Task 6's
// V13->V14 one). AcquisitionPage is now typed on CalculatorInputsV14, so
// every fixture is migrated once, here, rather than widening the shared
// fixture file for one caller.
//
...
// an already-v13 one, and `migrateV13toV14` an already-v14 one, so the day
// memo-fixtures.ts is widened to return CalculatorInputsV14 -- making these
// calls redundant -- every wrapper below throws immediately and every test
// in this file fails at setup, not a silent double-migration or a quietly
// wrong document.
function welshInputs(): CalculatorInputsV14 {
  return migrateV13toV14(migrateV12toV13(migrateV11toV12(migrateV10toV11(migrateV9toV10(migrateV8toV9(migrateV7toV8(migrateV6toV7(welshInputsV6()))))))));
}
function scottishInputs(): CalculatorInputsV14 {
  return migrateV13toV14(migrateV12toV13(migrateV11toV12(migrateV10toV11(migrateV9toV10(migrateV8toV9(migrateV7toV8(migrateV6toV7(scottishInputsV6()))))))));
}
function unconfirmedJurisdictionInputs(): CalculatorInputsV14 {
  return migrateV13toV14(migrateV12toV13(migrateV11toV12(migrateV10toV11(migrateV9toV10(migrateV8toV9(migrateV7toV8(migrateV6toV7(unconfirmedJurisdictionInputsV6()))))))));
}
...
function derivedUnconfirmed(base: CalculatorInputsV14): CalculatorInputsV14 {
  return {
    ...base,
    acquisition: {
      ...base.acquisition,
...
describe('AcquisitionPage — the jurisdiction control (R8 defect B)', () => {
  ...
  // defaultCalculatorInputsV14 deliberately records no jurisdiction of its own.
  it('says a defaulted jurisdiction was never recorded, and flags it unconfirmed', () => {
    const base = welshInputs();
    const migrated: CalculatorInputsV14 = {
      ...base,
      acquisition: {
        ...base.acquisition,
  ...
  it('does not claim a confirmed basis when the acquisition date is still outstanding', () => {
    const base = welshInputs();
    const noDate: CalculatorInputsV14 = {
...
describe('AcquisitionPage — the acquisition date', () => {
  ...
  it('surfaces the engine\'s own error for a date no band set covers', () => {
    const base = welshInputs();
    const tooEarly: CalculatorInputsV14 = {
...
describe('AcquisitionPage — the tax override', () => {
  function overridden(pence: number | null, reason: string): CalculatorInputsV14 {
    const base = welshInputs();
    return {
      ...base,
      acquisition: {
