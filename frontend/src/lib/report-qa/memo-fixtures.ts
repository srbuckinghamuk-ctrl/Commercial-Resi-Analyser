...
export function sellAllInputs(): CalculatorInputsV4 {
  return {
    ...
    scenarios: {
      ...
      upside: { label: 'Upside', gdv_adjustment_pct: 8, construction_cost_adjustment_pct: -5, timeline_adjustment_months: -2, interest_rate_adjustment_pct: 0, phase_slip_phase_id: null, phase_slip_months: 0, exit_yield_adjustment_pct: 0, operating_cost_adjustment_pct: 0, vacancy_adjustment_pct: 0, sales_slip_months: 0, saleable_area_adjustment_pct: 0, abnormal_cost_adjustment_pct: 0, programme_slip_months: 0, refi_ltv_adjustment_pct: 0 },
      downside: { label: 'Downside', gdv_adjustment_pct: -10, construction_cost_adjustment_pct: 12, timeline_adjustment_months: 3, interest_rate_adjustment_pct: 1, phase_slip_phase_id: null, phase_slip_months: 0, exit_yield_adjustment_pct: 0, operating_cost_adjustment_pct: 0, vacancy_adjustment_pct: 0, sales_slip_months: 0, saleable_area_adjustment_pct: 0, abnormal_cost_adjustment_pct: 0, programme_slip_months: 0, refi_ltv_adjustment_pct: 0 },
      severe: { label: 'Severe', gdv_adjustment_pct: -18, construction_cost_adjustment_pct: 20, timeline_adjustment_months: 6, interest_rate_adjustment_pct: 2, phase_slip_phase_id: null, phase_slip_months: 0, exit_yield_adjustment_pct: 0, operating_cost_adjustment_pct: 0, vacancy_adjustment_pct: 0, sales_slip_months: 0, saleable_area_adjustment_pct: 0, abnormal_cost_adjustment_pct: 0, programme_slip_months: 0, refi_ltv_adjustment_pct: 0 },
    },
    deal_spider: {
      storeys: 3,
...
/**
 * R14 (Task 12, spec §9/§20.4). The release's golden monitoring case: a v11
 * document (the first v11-native fixture — inputs_version 11 already, so
 * `migrateInputsToV11` takes the merge branch and preserves the `monitoring`
 * block untouched, per that function's own `saved.monitoring ?? null` line)
 * with a non-null `monitoring` block set at reporting month 6. Every OTHER
 * fixture this file exports carries `monitoring: null` (pre-v11 documents
 * migrate to it, and no other v11 fixture exists here), which is exactly what
 * lets `memo-release-gate.test.ts`'s ROUTES sweep assert the memo's new
 * "Monitoring cost-to-complete" section is absent everywhere except the
 * document built from this function.
 *
 ...
 * NOT hold for.
 */
export function monitoringOnSiteInputs(): CalculatorInputsV11 {
  const raw = JSON.parse(
 ...
 * moved it on to v14 (spec §24.8) for the same reason -- the memo release
