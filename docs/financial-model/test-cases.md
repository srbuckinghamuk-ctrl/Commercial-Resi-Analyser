...
**Status:** Authoritative test-case register for calculation specification `2.19.0` (see
...
## 27. The elemental cost benchmark [R17 — calc 2.19.0]

### 27.1 Fixture AB — the elemental cost benchmark (`fixtures/financial-model/ab-elemental-benchmark.json`)

**Purpose:** the release's golden case for spec §27 — the benchmark layer's
currentisation, location and adjustment arithmetic (§27.3), the result block and its
totals (§27.4), the warnings (§27.5), the forward-inflation disclosure read from §24.3's
factors, and the **advisory proof**: the appraisal's metrics are identical with and
without the benchmark block. `kind: "pipeline"`, authored at v17, named
`AB — elemental benchmark, TEST FIXTURE — NOT MARKET DATA`. Every rate in it is
fictional; the set is never seeded into the library. It is generated by a script that
derives every benchmark figure by arithmetic from the spec's formulas without importing
either engine (only the v17 migration and the content-hash helper are called, and the
hash is itself pinned by parity across both engines).

**The document.** Fixture Z (`z-cost-plan-in-time`: a detailed cost plan of five
packages, a precedence-network programme, a 6% p.a. inflation allowance, 5% general
contingency, `developed_area_sqm` 600) migrated to v17, with one change to Z's own
inputs — `cost_plan.qs.base_date` moved from Z's date to `2026-06-01`, the set's
`currentisation_date`, because the fixture carries an application and spec §27.3's seam
requires the two to be equal — and the benchmark block below added. The sixth package,
`pkg-bm-kitchens`, is the product of that application.

| Fact | Value | Source |
|---|---|---|
| Provider | `user_qs` → label `User/QS benchmark` | `set.provider_type`, spec §27.1 |
| Base date / currentisation date | `2025-06-01` / `2026-06-01` | `set.base_date`, `set.currentisation_date` |
| Index | `TEST index` 120.0 at base, 126.0 current | `set.base_index_value`, `set.current_index_value` |
| Currentisation factor | 126.0 ÷ 120.0 = **1.05** exactly | spec §27.3 |
| Location factor / multiplier | 95 → **0.95** | `set.location_factor`, spec §27.3 |
| Combined factor on every currentised rate | 1.05 × 0.95 = **0.9975** | — |
| Area basis for every area row | 600 m² | `developed_area_sqm` (spec §15.4) |
| Thresholds | material variance **10%** (not the default 15), stale 12 months, coverage 60% | `thresholds` |
| QS packages (Z's five, base-date amounts) | enabling 6,000,000; structure 24,000,000; envelope 18,000,000; M&E 12,000,000; externals 6,000,000 | Z's `cost_plan.packages` |
| Applied package | `pkg-bm-kitchens`, `finishes`, 3,192,000, `price_basis: estimate`, `benchmark_origin` (`r-kitchens`, method `index_ratio`) | the application |
| `months_from_base` for §24.3 | `2026-06-01` → month 0 (`2026-08-01`) = 2 | spec §24.3 |
| Content hash | `087c57e76aeb98e5814faf68991481d579a28ad4c650c186f4d38acd9e3d6baf` | `benchmark_content_hash`, spec §27.6 |

**The seven rates and selections**, and the hand derivation of every row (spec §27.3;
`round_half_up` is the one rounding, applied to the amount only):

| Row (catalogue order) | Rate as entered | Canonical p/m² (or per unit) | Currentised (× 0.9975) | Adjustment | Adjusted | Quantity | Amount (pence) |
|---|---|---|---|---|---|---|---|
| `strip_out` (`r-strip`) | 8,000 p/m² (`gbp_per_sqm`); LQ 7,000, median 8,000, UQ 9,000, n = 10 | 8,000 | 7,980 | 0 | 7,980 | 600 m² | 7,980 × 600 = **4,788,000** |
| `frame_alterations` (`r-frame`) | 2,500 p/ft² (`gbp_per_sqft`) | 2,500 × 10.7639104167097 = 26,909.77604177425 | 26,842.501601669814 | 0 | 26,842.501601669814 | 600 m² | 16,105,500.961… → **16,105,501** |
| `external_walls_facade` (`r-facade`, `unverified`) | 25,000 p/m² | 25,000 | 24,937.5 | **−10%** ("Façade largely retained; scope reduced") | 22,443.75 | 600 m² | 22,443.75 × 600 = **13,466,250** |
| `kitchens` (`r-kitchens`, per unit) | 800,000 p per dwelling (`gbp_per_unit`) | 800,000 | 798,000 | 0 | 798,000 | 4 units | 798,000 × 4 = **3,192,000** |
| `mechanical_services` (`r-mech`) | 18,000 p/m²; LQ 20,000, median 22,000, UQ 25,000, n = 6 | 18,000 | 17,955 | 0 | 17,955 | 600 m² | 17,955 × 600 = **10,773,000** |
| `preliminaries` (`r-prelims`, percentage) | 12.0% | — | never currentised | 0 | — | subtotal | see below |
| `external_works` (`r-ext`, lump sum, `estimated`) | 5,000,000 p (`gbp`) | 5,000,000 | 4,987,500 | 0 | 4,987,500 | 1 | **4,987,500** |

The rows publish in **catalogue** order — `strip_out`, `frame_alterations`,
`external_walls_facade`, `kitchens`, `mechanical_services`, `preliminaries`,
`external_works` — not in the order the selections were entered (spec §27.4).

**Subtotal and the percentage row.**

```
elemental_subtotal_pence = 4,788,000 + 16,105,501 + 13,466,250 + 3,192,000 + 10,773,000 + 4,987,500
                         = 53,312,251
preliminaries            = round_half_up(53,312,251 × 12 / 100) = round_half_up(6,397,470.12) = 6,397,470
benchmark_base_construction_pence = 53,312,251 + 6,397,470 = 59,709,721
```

**Totals.**

```
qs_base_construction_pence = 66,000,000 (Z's five packages) + 3,192,000 (the applied kitchens package) = 69,192,000
difference_pence           = 59,709,721 − 69,192,000 = −9,482,279
difference_pct             = −9,482,279 ÷ 69,192,000 × 100 = −13.7043… → −13.7
benchmark_rate_pence_per_sqm = round_half_up(59,709,721 ÷ 600) = round_half_up(99,516.2) = 99,516
qs_rate_pence_per_sqm        = round_half_up(69,192,000 ÷ 600) = 115,320   (= cost_plan.implied_rate_pence_per_sqm)
mapped_qs_amount_pence   = 69,192,000 (every package is targeted by exactly one row); unmapped = 0
unpriced_elements = 0;  elements_without_evidence = 2 (facade unverified, external works estimated)
elements_outside_range = 1 (mechanical: 17,955 < currentised LQ 20,000 × 0.9975 = 19,950;
                            strip-out sits inside 6,982.5 … 8,977.5 → false; the other rows have no quartiles → null)
coverage_pct = 6 priced non-percentage elements ÷ 34 catalogue non-percentage elements = 17.647… → 17.65
```

**Per-row QS comparison** (detailed mode: each row's `qs_amount_pence` is its target
package's whole base-date amount; `variance = benchmark − qs`):

| Row | Target | QS amount | Variance |
|---|---|---|---|
| strip_out | pkg-enabling | 6,000,000 | −1,212,000 |
| frame_alterations | pkg-structure | 24,000,000 | −7,894,499 |
| external_walls_facade | pkg-envelope | 18,000,000 | −4,533,750 |
| kitchens | pkg-bm-kitchens | 3,192,000 | 0 |
| mechanical_services | pkg-mande | 12,000,000 | −1,227,000 |
| preliminaries | — | null | null |
| external_works | pkg-externals | 6,000,000 | −1,012,500 |

**Warnings** (spec §27.5), exactly three, in the engine's emission order:
`incomplete_coverage` (17.65 < 60), `material_variance` (|−13.7| > 10 — **red**; this
is why the fixture lowers the threshold from 15: at the default the flag would not
fire and the threshold would be untested as an input), `source_unverified` (the façade
rate). Not raised: `benchmark_stale` (base to currentisation is exactly 12 months, not
more than 12), `no_location_evidence`, `project_type_mismatch`, `rates_not_currentised`,
`unpriced_elements`, `applied_without_currentisation` (the origin records
`index_ratio`).

**The six §24.3 forward factors from base `2026-06-01`.** With the base two months
before month 0, a package whose curve-weighted midpoint is `m` is inflated over
`2 + m` months at 6% p.a. compounded pro rata, `factor = 1.06^((2 + m)/12)`:

| Package | Midpoint (Z, spec §24.2) | Months | Factor | `inflation_pence` = round_half_up(amount × (factor − 1)) |
|---|---|---|---|---|
| pkg-enabling | 6.5 | 8.5 | 1.042137414046715 | 252,824 |
| pkg-structure | 10.5 | 12.5 | 1.062576670087961 | 1,501,840 |
| pkg-envelope | 10.5 | 12.5 | 1.062576670087961 | 1,126,380 |
| pkg-mande | 12.333333333333 | 14.333… | 1.072078163381325 | 864,938 |
| pkg-externals | 10.5 | 12.5 | 1.062576670087961 | 375,460 |
| pkg-bm-kitchens | 10.5 | 12.5 | 1.062576670087961 | 199,745 |

```
inflation_total_pence      = 4,321,187
contingency_total_pence    = round_half_up(69,192,000 × 5 / 100) = 3,459,600
construction_total_pence   = 69,192,000 + 4,321,187 + 3,459,600 = 76,972,787
```

The forward-inflated benchmark amounts (a disclosure, entering nothing) are each row's
amount × its target's factor, rounded once: strip-out 4,989,754; frame 17,113,330; façade
14,308,923; kitchens 3,391,745; mechanical 11,549,498; preliminaries null (no target);
external works 5,299,601.

**Content hash and the normalisation rule** (spec §27.6). The hash is `canonical_hash`
over the set with `content_hash`, `id`, `created_at` and `imported_by` removed and the
seven rates sorted by `element_code` then `id`; the fixture's value is
`087c57e76aeb98e5814faf68991481d579a28ad4c650c186f4d38acd9e3d6baf`, pinned in both
engines, republished on the result and carried by the kitchens package's
`benchmark_origin.set_content_hash` and by the application record.

### 27.2 Unit invariance — 620 m² × £1,250 and the ft² twin

Spec §27.2's worked invariant, asserted in both engines on a standalone set and on
every area row of fixture AB:

```
metric:    620 m² × 125,000 p/m²                           = 77,500,000 p → round_half_up → 77,500,000
imperial:  620 × 10.7639104167097 = 6,673.62445836 ft²  (8 dp; printed, never stored)
           125,000 ÷ 10.7639104167097 = 11,612.88 p/ft²   (2 dp; printed, never stored)
           6,673.62445836… × 11,612.8800… = 77,500,000.0 → round_half_up → 77,500,000
```

The same rate entered as `gbp_per_sqft` at `11,612.88…` p/ft² is converted once, on
entry, to the unrounded canonical `125,000` p/m² and produces the identical amount. On
AB, `frame_alterations` is the row that carries this: entered at 2,500 p/ft², canonical
26,909.77604177425 p/m², amount 16,105,501 — the same figure the row yields when the
rate is re-expressed per m². The round-trip test toggles the AB document 1,000 times and
asserts every canonical value bit-identical, and the entry-precision test pins that
`1,076.39 ft²` stores `99.9999 m²` (100 m² being 1,076.391 ft²).

### 27.3 What is hand-derived and what is identity-asserted

Every figure in §27.1 above — each row's canonical, currentised and adjusted rate, each
amount, the subtotal, the percentage row, the totals, the per-row variances, the six
forward factors, the three warnings, the coverage and the content hash — is worked out
**by hand, on this page, before either engine runs**, and is what fixture AB's
`expected_metrics` pins (`benchmark_row_*`, `elemental_benchmark.totals.*`,
`cost_plan.*`, `package_inflation_*`).

The ledger-dependent figures — TDC, peak debt, profit, LTC/LTGDV and the rest of the
metric set — are, by contrast, **never pinned as numbers here**, exactly fixture AA's
rule (§25.5, and fixture K's before it, model-governance.md §2.1). Spec §27 *defines*
the benchmark layer as advisory (`enters_tdc: false`), so the test that proves it
asserts the contract itself: fixture AB run with its `elemental_benchmark` block
replaced by `null` (and the kitchens package left in place, because an application is
an ordinary cost-plan edit) yields metrics identical to the run with the block present
on every field but `elemental_benchmark` — `construction_total_pence`, TDC, peak debt
and profit named in the assertion. A first draft that added the benchmark total to
construction was watched red by this test before the result block existed.

### 27.4 Test counts

Re-run on 30 August 2026, both green:

- **vitest** — `npx vitest run src/lib/model/elemental-benchmark.test.ts src/lib/area-units.test.ts src/lib/sha256.test.ts`: **49 tests, 3 files**.
- **pytest** — `python -m pytest -q tests/test_elemental_benchmark.py tests/test_area_units.py tests/test_migrate_v17.py`: **94 passed** (26 elemental benchmark, 12 area units, 56 migrate-v17 — the migration file is parametrised over the corpus).

AB is the only new file under `fixtures/financial-model/`; it is authored at v17 and is
excluded from the migration gates that predate its schema, as Z is from those that
predate its own. No existing fixture pin's value moves in this release.
