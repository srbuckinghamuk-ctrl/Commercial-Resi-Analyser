...
`developed_area_sqm(inputs)` (`developedAreaSqm` in TypeScript) is the **only** way any consumer may obtain the construction cost area — the cost stack, validation, the deal spider, the UI and the investment memo all call it and nothing else. Reading `conversion_costs.total_construction_sqm` directly is a **build failure** outside a short allowlist of files that own, declare, migrate or capture the raw field.
