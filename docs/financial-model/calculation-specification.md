...
8. `cost_plan.qs` non-null in headline mode is an error (§17.1's shape). When present: `source` non-empty, `stage` and `status` in their enums, and both dates real (rule 5).
...
**The schedule is §12.2-invariant.** No lever writes to `due_diligence`, so every sensitivity cell carries the identical schedule, the identical derived rows and the identical `due_diligence_unknown`, `source_conflict` and `consent_expires_before_start` flags as the base cell. Only `provisional_sums_present`'s **amount** can move, because the cost lever scales the packages it is summed from. There is no due-diligence lever: a "risks crystallise" stress (Σ cost impact onto construction, Σ programme impact onto the timeline) is recorded for R16's presets, not built here.
