...
Geometric gates (`memo-release-gate.test.ts`: page bounds, sparse pages, orphaned headings, footer, layout determinism) ran over every route including the new AB route (19 pages) on both metric and imperial renders and passed. The live York memorandum was rendered page by page (pdf.js in Chrome, both the metric and the imperial export, 16 physical pages each): no text overflow, no clipped or orphaned heading, no effectively blank page, the DRAFT watermark on every page; the provenance panel prints calc 2.19.0, inputs v17, an audit hash, the lender-case row and `Report area unit` = `m²` / `ft² (1 m² = 10.7639104167097 ft²)`; the peak-debt sentence ("Month 11") and the cashflow table's peak row agree; "Unrealised Return on Equity" on three pages; the refinance stress row reads "Maximum refinance LTV reduced by 10.0 percentage points."; the PDF/UA limitation is item 6; no Elemental Cost Benchmark section on York; the audit hash is identical across the two units. **One functional defect was found and fixed in the same session:** the memo printed only the legacy listing-record conflicts and said "source-conflict checks did not run" although the structured field conflict existed; it now prints every unresolved field conflict as an Information Required line (test added). Cosmetic findings left open and recorded: a limitations row and the finance-cost group each split across a page boundary under repeated headers; §2's dash placeholders for storeys and floor area when the project row lacks them; a wrapped "51 m²" cell; inapplicable stress rows joining setting and reason without a separator; the watermark largely covered by table fills on table-heavy pages.
