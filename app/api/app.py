...
from app.financial_model.migrate import is_v2_or_later, migrate_inputs_to_v13
...
def calculate_authoritative(
    ...
    try:
        ...
        # migrate_inputs_to_v6 and migrate_inputs_to_v5 before it, and the merge helper all three delegate to -- raises plain Python
    ...
    return {
        ...
        # `migrate_inputs_to_v12`; R15 Task 13 moves it on again to
        # `migrate_inputs_to_v13`; this line needs no change at all either
        # time, which is the point of deriving it. `inputs` is already a
        # `CalculatorInputsV13` here (the `migrate_inputs_to_v13(raw)` call
        # above), whose `inputs_version` field is `Literal[13] = 13` -- the
