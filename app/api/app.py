...
from app.financial_model.migrate import is_v2_or_later, migrate_inputs_to_v12
...
def calculate_authoritative(
    ...
    try:
        ...
        # migrate_inputs_to_v5 before it, and the merge helper all three delegate to -- raises plain Python
    ...
    return {
        ...
        # `migrate_inputs_to_v11`; R13b Task 15 moves it on again to
        # `migrate_inputs_to_v12`; this line needs no change at all either
        # time, which is the point of deriving it. `inputs` is already a
        # `CalculatorInputsV12` here (the `migrate_inputs_to_v12(raw)` call
        # above), whose `inputs_version` field is `Literal[12] = 12` -- the
